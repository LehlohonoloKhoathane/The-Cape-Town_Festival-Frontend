...
export default {
  ...
  async mounted() {
    try {
      ...
      if (RSVPs && RSVPs.length > 0) { // Check if RSVPs data is not null and has at least one item
        const rentalDurations = {}; // Object to store RSVP durations for each date
        RSVPs.forEach((RSVP) => {
          const issuedDate = RSVP.issuedDate ? RSVP.issuedDate.split('T')[0] : null; // Extract date from timestamp
          const returnedDate = RSVP.returnedDate ? RSVP.returnedDate.split('T')[0] : null; // Extract date from timestamp
