        ...
        <div class="form-group">
          ...
          <select id="event" v-model="selectedEvent" name="eventId">
            <option v-for="event in events" :key="event.id" :value="event.id">
              Event ID: {{ event.id }} Make: {{ event.name }} Model: {{ event.model }}
            </option>
          </select>
        </div>
    ...
    createRental() {
      ...
      axios
          ...
          .then(response => {
            ...
            RSVP.user.id = userData.id;
            RSVP.user.firstName = userData.firstName;
            RSVP.user.lastName = userData.lastName;
            RSVP.user.email = userData.email;
            RSVP.user.enabled = userData.enabled;
            RSVP.user.roles = userData.roles; // Include user roles
            console.log("User Details filtered from response: ");
            console.log(RSVP.user);

            // Fetch event details
            axios
                .get(`/api/admin/events/read/${this.selectedEvent}`, {
                  headers: {
                    Authorization: `Bearer ${token}`
                  }
                })
                .then(response => {
                  RSVP.event = response.data;
                  ...
                  this.confirm = () => {
                    ...
                    // Make the POST request to create the RSVP
                  /*  console.log(RSVP.userObject);*/
                    axios
                        ...
                        .then(response => {
                          ...
                          // Reset the form after creating the RSVP
                          this.resetForm();
                        })
                        .catch(error => {
                          console.log(error);
                          if (error.response && error.response.status === 400) {
                            ...
                            this.errorMessage = 'An error occurred while creating the RSVP.';
                            this.failModal.message = error.response.data;
                            this.failModal.show = true;
                          }
                          this.loadingModal.show = false;
